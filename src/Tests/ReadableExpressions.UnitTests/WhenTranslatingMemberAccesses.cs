...
string.Join(
    ...
    [Fact]
    public void ShouldTranslateADictionaryAccessExpression()
    {
        var keyedAccess = CreateLambda(
            (Dictionary<string, string> d, string key) => d[key + "Key"]);

        var translated = keyedAccess.Body.ToReadableString();

        translated.ShouldBe("d[key + \"Key\"]");
    }
