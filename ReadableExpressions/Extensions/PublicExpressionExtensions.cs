...
public static class PublicExpressionExtensions
{
    ...
    public static Expression GetParentOrNull(this Expression expression)
    {
        ...
        {
            ExpressionType.ArrayIndex => ((BinaryExpression)expression).Left,
            ExpressionType.Index => ((IndexExpression)expression).Object,
            ExpressionType.Call => ((MethodCallExpression)expression).GetSubject(),
            ExpressionType.MemberAccess => ((MemberExpression)expression).Expression,
            _ => null
        };
    }
